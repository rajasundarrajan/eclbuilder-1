...
import org.hpccsystems.dsp.dashboard.controller.DashboardGrid;
...
public class HomeController extends HomeComposer {
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		/*dummy.addAll(((DSPDao) SpringUtil.getBean("dspDao"))
				.getECLBuilders(((AuthenticationService) SpringUtil.getBean(Constants.AUTHENTICATION_SERVICE))
...
//		entityList.setModel(dummy);
		...
		thumbnailLayout.addEventListener(EVENTS.ON_CLICK_VIEW_OR_EDIT,
				event -> openDashboard((DashboardConfig) event.getData()));

		thumbnailLayout.addEventListener(EVENTS.ON_CLICK_CLONE,
				event -> cloneComposition((DashboardConfig) event.getData()));

		thumbnailLayout.addEventListener(EVENTS.ON_DELETE_COMPOSITION,
				event -> initiateDashboardDelete((Dashboard) event.getData()));

		thumbnailLayout.addEventListener(EVENTS.ON_FAV_COMPOSITION,
				event -> markAsFavouriteDashbaord((Dashboard) event.getData()));

		homeTabbox.addEventListener(Dashboard.EVENTS.ON_OPEN_DASHBOARD, event -> openECLBuilderTab());
	...
	public void reloadBuilderList() {
		try {
			...
			Column col= new Column(Labels.getLabel("dspName"));
			 cols.appendChild(col); 
			 col= new Column(Labels.getLabel("byDate"));
			 cols.appendChild(col);
			 col= new Column(Labels.getLabel("byAuthor"));
			 cols.appendChild(col);
			 col= new Column(Labels.getLabel("dspActions"));
			 cols.appendChild(col);
			 ...
			 for(Builder build : dummy){
				 ...
				 button1.addEventListener("onClick", event -> {editECLBuilder(event);});
				 ...
				 button1.addEventListener("onClick", event -> {cloneECLBuilder(event);});
				 button1.setZclass("img-btn");
				 ...
				 button1.addEventListener("onClick", event -> {deleteECLBuilder(event);});
...
//			entityList.renderAll();
	...
	private void generateGridHeader() throws ClassNotFoundException, InstantiationException, IllegalAccessException {
		...
		try {
			...
			Clients.showNotification(Labels.getLabel("sortingNotAvailableInList"), Clients.NOTIFICATION_TYPE_ERROR,
	...
	private void createNewDashboardWindow(DashboardConfig config) {
		...
		String userAction = (String) Executions.getCurrent().getAttribute("userAction");
		Window window = (Window) Executions.createComponents(NEW_DASHBOARD_URI, getSelf(), newDashboardData);
		window.setAttribute("userAction", userAction);
		window.doModal();
	}
	...
	public void promoteMessageBox(String promoteBeforFunction) {
		Messagebox.show(Labels.getLabel(PROMOTE_CONFIRMATION_MESSAGE) + "?",
				Labels.getLabel(PROMOTE_CONFIRMATION_HEADER),
				new Messagebox.Button[] { Messagebox.Button.YES, Messagebox.Button.NO }, Messagebox.QUESTION,
				(EventListener<ClickEvent>) event -> confirmPromote(event, promoteBeforFunction));
...
//	@Listen("onClick = .cloneButton")
	public void cloneECLBuilder(Event e) {

		Executions.getCurrent().setAttribute("userAction", "clone");
		Executions.getCurrent().setAttribute("builderName", e.getTarget().getParent().getParent().getId());
		DashboardConfig dashboardConf = new DashboardConfig();
		dashboardConf.setDashboard(new Dashboard());
		cloneComposition(dashboardConf);

		/*
		 * try { String builderName =
		 * ((Button)e.getTarget()).getParent().getId(); String userID =
		 * ((User)((AuthenticationService)
		 * SpringUtil.getBean(Constants.AUTHENTICATION_SERVICE)).getCurrentUser(
		 * )).getId(); List<Builder> eclBuilders;
		 * 
		 * eclBuilders =
		 * ((DSPDao)SpringUtil.getBean("dspDao")).getECLBuilder(userID,
		 * builderName);
		 * 
		 * if(eclBuilders.size() == 0){ return; } Builder selectedBuilder =
		 * eclBuilders.get(0); Tab tab = new Tab(); Tabpanel tabpanel = new
		 * Tabpanel(); Include include = new Include();
		 * include.setId("eclBuilderInclude");
		 * Executions.getCurrent().setAttribute("hpccConnId",Executions.
		 * getCurrent().getAttribute("hpccConnID"));//,
		 * Executions.getCurrent().getParameter("dashboardType"));
		 * Executions.getCurrent().setAttribute("BuilderName",Executions.
		 * getCurrent().getAttribute("BuilderName"));
		 * include.setDynamicProperty("dashboardType",
		 * Executions.getCurrent().getAttribute("dashboardType"));
		 * include.setDynamicProperty("selectedBuilder", selectedBuilder);
		 * include.setDynamicProperty("userAction", "clone");
		 * include.setSrc("eclBuilder/fileBrowser.zul");
		 * 
		 * tabpanel.appendChild(include); homeTabbox.getTabs().appendChild(tab);
		 * homeTabbox.getTabpanels().appendChild(tabpanel);
		 * 
		 * tab.setSelected(true); } catch (DatabaseException e1) { // TODO
...
//	@Listen("onClick = .editButton")
	public void editECLBuilder(Event e) {
		...
		try {
			String builderName = e.getTarget().getParent().getParent().getId();

			if (((Tabs) homeTabbox.getChildren().get(0)).getChildren().stream()
					.filter((comp) -> ((Tab) comp).getLabel().equals(builderName)).count() > 0) {
				Tab tabWithEditBuilder = (Tab) ((Tabs) homeTabbox.getChildren().get(0)).getChildren().stream()
						.filter((comp) -> ((Tab) comp).getLabel().equals(builderName)).findFirst().get();
				tabWithEditBuilder.setSelected(true);
				return;
			}
			...
			eclBuilders = ((DSPDao) SpringUtil.getBean("dspDao")).getECLBuilder(userID, builderName, "");
...
//			("updateHomeGrid", include, listener)
	...
	public void deleteECLBuilder(Event e) {
		String builderName = e.getTarget().getParent().getParent().getId();
		String userId = ((AuthenticationService) SpringUtil.getBean(Constants.AUTHENTICATION_SERVICE)).getCurrentUser()
				.getId();
		try {
			((DSPDao) SpringUtil.getBean("dspDao")).deleteECLBuilder(userId, builderName);
			reloadBuilderList();
		} catch (DatabaseException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
	...
	public void dashboardSharedByMe() {
		try {
			doAfterFilteringDashboards(((CompositionService) SpringUtil.getBean(Constants.COMPOSITION_SERVICE))
					.filterDashboards(user, true, false, getCompositions()));
	...
	public void dashboardCreatedByMe() {
		try {
			doAfterFilteringDashboards(((CompositionService) SpringUtil.getBean(Constants.COMPOSITION_SERVICE))
					.filterDashboards(user, true, true, getCompositions()));
		} catch (CompositionServiceException e) {
			LOGGER.error(Constants.EXCEPTION, e);
			showErrorNotification(e.getMessage());
			return;
	...
	private void doAfterFilteringDashboards(List<Dashboard> list) {
		...
		try {
			...
			((CompositionService) SpringUtil.getBean(Constants.COMPOSITION_SERVICE))
					.assignClusterToDshboard(dashboards);
	...
	private void initiateDashboardDelete(Dashboard dashboard) throws HipieException, DatabaseException {
		...
		if (!HipieSingleton.getHipie().getPermissionsManager().userCanEdit(user.getId(), dashboard.getComposition())) {
			Clients.showNotification(Labels.getLabel("youDonotHavePermissionToDeleteDashboard"),
					Clients.NOTIFICATION_TYPE_ERROR, homeTabbox, Constants.POSITION_MIDDLE_CENTER, 3000);
			return;
		}
		if (dashboard.isRunning()) {
			Clients.showNotification(Labels.getLabel("cannotDeleteDashboardWhileRunning"),
					Clients.NOTIFICATION_TYPE_ERROR, getSelf().getParent(), Constants.POSITION_TOP_CENTER, 5000, true);
			return;
		}

		ContractInstance contractInstance = CompositionUtil
				.getVisualizationContractInstance(dashboard.getComposition());

		if (contractInstance != null && !CompositionUtil.extractRunOption(contractInstance)
				&& ((CompositionService) SpringUtil.getBean(Constants.COMPOSITION_SERVICE))
						.getmostRecentInstance(dashboard.getComposition(), true) != null) {
			...
			Messagebox.show(Labels.getLabel("deleteConfirmationService") + Constants.SPACE + dashboard.getLabel() + "?",
					...
					new String[] { Labels.getLabel("deleteEverything"), Labels.getLabel("onlyDashboard"),
							Labels.getLabel("cancel") },
			...
			Messagebox.show(Labels.getLabel("deleteConfirmation") + Constants.SPACE + dashboard.getLabel() + "?",
					Labels.getLabel(DELETE_CONFIRMATION_DIALOGUE),
					new Messagebox.Button[] { Messagebox.Button.YES, Messagebox.Button.NO }, Messagebox.QUESTION,
					(EventListener<ClickEvent>) event -> confirmDeleteDashboard(dashboard, startTime, event));
		}
	...
	private void deleteDashboard(Dashboard dashboard, int startTime, boolean deleteservices) {
		...
		try {
			...
			((CompositionService) SpringUtil.getBean(Constants.COMPOSITION_SERVICE))
					.deleteAccessLog(dashboard.getComposition().getId(), user.getId(), false);

			((CompositionService) SpringUtil.getBean(Constants.COMPOSITION_SERVICE)).deleteComposition(
					dashboard.getComposition(),
					((AuthenticationService) SpringUtil.getBean(Constants.AUTHENTICATION_SERVICE)).getCurrentUser()
							.getId(),
					deleteservices);
			LOGGER.debug("compositionService.deleteComposition() end duration --->{}",
		...
		LOGGER.debug("updateDashboardList() end duration --->{}",
				Calendar.getInstance().get(Calendar.SECOND) - seconds);

		seconds = Calendar.getInstance().get(Calendar.SECOND);
		LOGGER.debug("notifyDashboardListUpdate() start --->{}");
		notifyDashboardListUpdate();
		LOGGER.debug("notifyDashboardListUpdate() end duration --->{}",
				Calendar.getInstance().get(Calendar.SECOND) - seconds);

		seconds = Calendar.getInstance().get(Calendar.SECOND);
		LOGGER.debug("updateRecentDashboards() start --->{}");
		updateRecentDashboards();
		LOGGER.debug("updateRecentDashboards() end duration --->{}",
				Calendar.getInstance().get(Calendar.SECOND) - seconds);

		LOGGER.debug("deleteComposition() End total time --->{}",
				Calendar.getInstance().get(Calendar.SECOND) - startTime);
	}
	...
	private void openDashboard(DashboardConfig config) {
		...
		try {
			...
			if (!config.getDashboard().isStaticData()) {
				CompositionInstance mostRecentCI = ((CompositionService) SpringUtil
						.getBean(Constants.COMPOSITION_SERVICE)).getmostRecentInstance(composition, true);
			...
			LOGGER.error(Constants.EXCEPTION, e);
			// All checked exception are from HIPIE, So displaying the message
			// directly
		...
		if (Flow.EDIT == config.getFlow()
				&& !HipieSingleton.getHipie().getPermissionsManager().userCanEdit(userId, composition)) {

			Clients.showNotification(Labels.getLabel("youDonotHavePermissionToEditDashboard"),
					Clients.NOTIFICATION_TYPE_ERROR, HomeController.this.getSelf(), Constants.POSITION_TOP_CENTER,
					3000);
			return;
		}
	...
	/*
	 * private void checkStaticdataAndOpen(DashboardConfig config, Composition
	 * composition) { if(config.isStaticData()) { openECLBuilderTab(config); }
	 * else { validateCompositionAndProceed(config, composition); try { boolean
	 * accessLog = ((CompositionService)
	 * SpringUtil.getBean(Constants.COMPOSITION_SERVICE))
	 * .logCompositionAccess(composition.getId(), user.getId()); if (!accessLog)
	 * { LOGGER.info(Labels.getLabel("notAbleToLogDashboardAccess")); } } catch
	 * (DatabaseException e) { LOGGER.error(Constants.EXCEPTION, e); } } }
	 */

	private boolean showCustomnotification(CompositionInstance mostRecentCI) {
		if (mostRecentCI == null) {
			Clients.showNotification(Labels.getLabel("processNotAvailableToView"), Clients.NOTIFICATION_TYPE_ERROR,
					this.getSelf(), Constants.POSITION_TOP_CENTER, 3000);
			return true;
		} else if (!DashboardUtil.isWorkUnitComplete(mostRecentCI)) {
			Clients.showNotification(Labels.getLabel("processNotComplete"), Clients.NOTIFICATION_TYPE_ERROR,
					this.getSelf(), Constants.POSITION_TOP_CENTER, 3000);
			return true;
		}
		return false;
	...
	/*
	 * private void validateCompositionAndProceed(DashboardConfig config,
	 * Composition composition) { //Validate the composition before proceeding
	 * boolean hasErrors =
	 * RampsUtil.checkCompositionError(composition,this.getSelf());
	 * 
	 * Dashboard dashboard = config.getDashboard();
	 * 
	 * if(!hasErrors){ if(dashboard.isStaticData()) { openECLBuilderTab(config);
	 * } else if (dashboard.getHpccConnection() == null) {
	 * createNewDashboardWindow(config); } else { try {
	 * if(dashboard.getHpccConnection().testConnection()){
	 * openECLBuilderTab(config); } } catch (Exception e) {
	 * LOGGER.error(Constants.EXCEPTION,e);
	 * Clients.showNotification(Labels.getLabel("hpccConnectionFailed")+": "
	 * +e.getMessage(), Clients.NOTIFICATION_TYPE_ERROR, this.getSelf(),
	 * Constants.POSITION_TOP_CENTER, 5000, true); } } } }
	 */
...
//		});
	...
	public void createDashboard(Event event) {
		try {
			...
			Executions.getCurrent().setAttribute("dashboardType", event.getTarget().getId());
			Window window = (Window) Executions.createComponents(NEW_DASHBOARD_URI, getSelf(), newDashboardData);
			window.doModal();
		} catch (Exception ex) {
	...
	protected void updateDashboardList(Dashboard dashboard) {
		...
		Clients.showNotification(Labels.getLabel("dashboardDeleted"), Clients.NOTIFICATION_TYPE_INFO, getSelf(),
	...
	private Collection<Dashboard> getSearchedDashboards() {
		...
		try {
			searched = getCompositions().stream()
					.filter(dashboard -> StringUtils.containsIgnoreCase(dashboard.getName(), searchTextBox.getValue())
							...
							|| StringUtils.containsIgnoreCase(dashboard.getLabel(), searchTextBox.getValue()))
					.collect(Collectors.toList());

			CompositionService compositionService = (CompositionService) SpringUtil
					.getBean(Constants.COMPOSITION_SERVICE);
	...
	public void filterProjects(InputEvent event) {
		...
		try {
			List<Dashboard> recentDashboard = getCompositions().stream()
					.filter(dashboard -> StringUtils.containsIgnoreCase(dashboard.getLabel(), event.getValue())
							...
							|| StringUtils.containsIgnoreCase(dashboard.getAuthor(), event.getValue()))
					.collect(Collectors.toList());
	...
	public void createNewFilteredProject(SelectEvent<Component, Dashboard> event) {
		...
		if (((AuthenticationService) SpringUtil.getBean(Constants.AUTHENTICATION_SERVICE)).getCurrentUser()
				.canViewProject()) {
	...
	public void sortByAuthor() {
		if (CollectionUtils.isEmpty(promotionEntities)) {
			...
			Messagebox.show(Labels.getLabel(PROMOTE_CONFIRMATION_MESSAGE) + "?",
					Labels.getLabel(PROMOTE_CONFIRMATION_HEADER),
					new Messagebox.Button[] { Messagebox.Button.YES, Messagebox.Button.NO }, Messagebox.QUESTION,
					(EventListener<ClickEvent>) event -> confirmSort(event, authorItemDashboard));
		}
	}
	...
	public void sortByDate() {
		if (CollectionUtils.isEmpty(promotionEntities)) {
			...
			Messagebox.show(Labels.getLabel(PROMOTE_CONFIRMATION_MESSAGE) + "?",
					Labels.getLabel(PROMOTE_CONFIRMATION_HEADER),
					new Messagebox.Button[] { Messagebox.Button.YES, Messagebox.Button.NO }, Messagebox.QUESTION,
					(EventListener<ClickEvent>) event -> confirmSort(event, dateItemDashboard));
		}
	}
	...
	public void sortByName() {
		if (CollectionUtils.isEmpty(promotionEntities)) {
			...
			Messagebox.show(Labels.getLabel(PROMOTE_CONFIRMATION_MESSAGE) + "?",
					Labels.getLabel(PROMOTE_CONFIRMATION_HEADER),
					new Messagebox.Button[] { Messagebox.Button.YES, Messagebox.Button.NO }, Messagebox.QUESTION,
					(EventListener<ClickEvent>) event -> confirmSort(event, nameItemDashboard));
		}
	}
	...
	private void toggleSortType(Menuitem item, Comparator<GridEntity> ascSort, Comparator<GridEntity> desSort,
			VIEW view) {
		...
		for (Component comp : homeTabbox.getTabs().getChildren()) {
			...
			try {
				if (tabDash != null && dashToOpen.getComposition().getId().equals(tabDash.getComposition().getId())) {
					exists = true;
					LOGGER.info(
							"Dashboard is opened in another tab :: " + dashToOpen.getComposition().getCanonicalName());
					break;
				}
			} catch (HipieException e) {
				LOGGER.error(Constants.ERROR, e);
